...
def notify_missing_timespentold():
    if config.is_enterprise:
        issues = graphql.get_project_issues_timespentold(
            owner=config.repository_owner,
            owner_type=config.repository_owner_type,
            project_number=config.project_number,
            timespentold_field_name=config.timespentold_field_name,
            filters={'empty_timespentold': True, 'closed_only': True}
        )
    else:
        # Get the issues
        issues = graphql.get_repo_closed_issues(
...
def notify_missing_estimateold():
    if config.is_enterprise:
        issues = graphql.get_project_issues_estimateold(
            ...
            owner_type=config.repository_owner_type,
            project_number=config.project_number,
            estimateold_field_name=config.estimateold_field_name,
            filters={'empty_estimateold': True, 'closed_only': True}
        )
    else:
        # Get the issues
        issues = graphql.get_repo_closed_issues(
